  ...
  public static getLaunchpadList = async (req: Request, res: Response) => {
    try {
      ...
      if (status !== 'admin' && status !== 'owner')
        WHERE =
          WHERE +
          ` AND ((SALE_START_TIME > TO_TIMESTAMP(${curTime}) AND STATUS = 'requested') OR (SALE_START_TIME < TO_TIMESTAMP(${curTime}) AND SALE_END_TIME > TO_TIMESTAMP(${curTime}) AND STATUS = 'approved') OR (SALE_END_TIME < TO_TIMESTAMP(${curTime}) AND STATUS = 'finished'))`
